...
var convertVirtualDom = function(node) {
	...
	// input[type=text]系要素のvalueをdefaultValueに変換
	_(node.querySelectorAll('input')).map(function(input) {
		...
		if (type !== 'checkbox' && type !== 'file' && type !== 'radio') {
			...
			if (attributes.getNamedItem('value')) {
				
				// IEではvalueの削除が許可されないので、直接新しいノードを生成する
				var newInput = document.createElement('input');
				for (var i = 0; i < attributes.length; i++) {
					var propName = attributes[i].name;
					if (propName === 'value') propName = 'defaultValue'
					newInput.setAttribute(propName, attributes[i].value);
				}
		...
		if (type === 'checkbox') {
			var checkedobj = input.attributes.getNamedItem('checked');
			input.setAttribute('defaultChecked', checkedobj ? checkedobj.value : null);
	...
	html = replaceAttribute(html, 'data-defaultValue',    'defaultValue');
	html = replaceAttribute(html, 'data-defaultChecked',  'defaultChecked');
	html = replaceAttribute(html, 'data-defaultSelected', 'defaultSelected');
